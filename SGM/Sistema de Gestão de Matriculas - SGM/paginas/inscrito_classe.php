...
<html lang="">
  ...
  <body>
    <div class="menu d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 bg-white border-bottom shadow-sm">
        ...
        <div id="descer">        
          <div class="table-responsive">
            <table class="table table-hover" >
              ...
              <?php
                $id = $_GET["id"];
                $lista_inscritos = mysqli_query($conexao,"SELECT * FROM matricula JOIN classe ON matricula.fk_classe = classe.id_classe JOIN curso ON matricula.fk_curso=curso.id_curso JOIN turno ON matricula.fk_turno=turno.id_turno JOIN encarregado ON matricula.fk_encarregado=encarregado.id_encarregado JOIN contacto ON matricula.fk_contacto=contacto.id_contacto JOIN provincia ON matricula.fk_provincia=provincia.id_provincia JOIN sexo ON matricula.fk_sexo=sexo.id_sexo JOIN morada ON matricula.fk_morada=morada.id_morada WHERE fk_classe='$id' ");
              ...
              <tr>
                <div style="position: fixed; top: 100px; right: 20%;">                 
                  <h5><?php echo $fk_classe?>Classe</h5>
                </div>
      ...
      <div>
        <?php 
          $total= mysqli_query($conexao, "SELECT COUNT(*) AS count FROM matricula WHERE fk_classe='$id'");
