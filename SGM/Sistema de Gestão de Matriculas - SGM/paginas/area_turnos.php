...
<html lang="">
  ...
  <body>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <p><span class="text-primary">Do lado esquerdo:</span><br> Encontra-se o formulário para adicionar novos turnos no sistema.</p>
                <p><span class="text-primary">Do lado direito:</span><br> Temos os turnos já adicionados no sistema.</p>
              </div>
    ...
    <div class="form p-3 px-md-4 mb-3 bg-white border-bottom shadow-lg p3 md-5 rounded">            
      <form action="../php/add_turno.php" method="post" autocomplete="off">
        <label>Adicionar Turno:</label>            
        <input type="text" name="turno" class="form-control" placeholder="Digite novo turno" required>
        ...
        <div class="table-responsive">
          <table class="table table-hover">
            <thead>
              ...
              <?php
                $lista_turno = mysqli_query($conexao,"SELECT * FROM turno");
                $total = mysqli_num_rows($lista_turno);                 
                while($resultado = mysqli_fetch_array($lista_turno)){
                $turno = $resultado['turno'];
              ?>
            <tbody>
              <tr>  
                <td><span class="cor"><?php echo $turno?></span></td>
                <td>
                  <?php echo "<a href='inscrito_turno.php?&id=".$resultado['id_turno']."'class='btn btn-success'>Inscrições</a>";?> 
                  <?php echo "<a href='ed_turno.php?&id=".$resultado['id_turno']."'class='btn btn-primary'>Editar</a>";?>                
                  <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampleModal<?php echo $resultado['id_turno']?>">Eliminar</button>

                    <!-- Modal -->
                    <div class="modal fade" id="exampleModal<?php echo $resultado['id_turno']?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Notificação</h5>
                          ...
                          <div class="modal-body">
                            Tens certeza que desejas eliminar este turno?
                          </div>
                          <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-dismiss="modal">Não</button>
                            <?php echo "<a href='../php/apagar_turno.php?&id=".$resultado['id_turno']."'class='btn btn-primary'>Sim</a>";?>
                          </div>
                        </div>
                      </div>
                    </div><!--Fim Modal---> 
