...
$select = mysqli_query($conexao,"SELECT matricula.*, fotos.* FROM matricula JOIN fotos ON (matricula.fk_foto = fotos.id_foto) WHERE matricula.id_matricula = $id_matricula");
...
if (isset($_POST['botao'])){
  if (isset($_FILES['foto'])){
    ...
    $nome = md5(time()).$foto;
    $pasta = '../imagens/fotos/';
    unlink('../imagens/fotos/'.$dados['foto']);
    move_uploaded_file($_FILES['foto']['tmp_name'],$pasta.$nome);
    $sql = mysqli_query($conexao,"UPDATE fotos SET foto = '$nome' WHERE id_foto = '".$dados['id_foto']."' ");
