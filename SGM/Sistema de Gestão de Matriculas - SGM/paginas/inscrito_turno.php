  ...
  <body>
    <div class="menu d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 bg-white border-bottom shadow-sm">
      ...
      <nav class="my-2 my-md-0 mr-md-3">
        <a class="p-2 text-dark" href="area_turnos.php">Fechar</a>
      </nav>
    </div>
      ...
      <div id="descer">
        <div class="table-responsive">
          <table class="table table-hover">
            ...
            <?php
              $id = $_GET["id"];
              $lista_inscritos = mysqli_query($conexao,"SELECT * FROM matricula JOIN classe ON matricula.fk_classe = classe.id_classe JOIN curso ON matricula.fk_curso=curso.id_curso JOIN turno ON matricula.fk_turno=turno.id_turno JOIN encarregado ON matricula.fk_encarregado=encarregado.id_encarregado JOIN contacto ON matricula.fk_contacto=contacto.id_contacto JOIN provincia ON matricula.fk_provincia=provincia.id_provincia JOIN sexo ON matricula.fk_sexo=sexo.id_sexo JOIN morada ON matricula.fk_morada=morada.id_morada WHERE fk_turno='$id' ");
            ...
            <tr>
              ...
              <td>
              <?php echo "<a href='../recibos/matricula.php?&id=".$resultado['id_matricula']."' class='btn btn-success'>Informações</a>";?>
              </td>                     
            </tr>
          </tbody>
          <?php
    ...
    <div>
        <?php 
          $total= mysqli_query($conexao, "SELECT COUNT(*) AS count FROM matricula WHERE fk_turno='$id'");
